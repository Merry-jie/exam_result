    ...
    if (this.state.hasError) {
      return (
        <div style={{
          ...
          Animation temporairement indisponible
        </div>
      );
    }
...
function App() {
  ...
  // États pour gérer le chargement de l'animation Spline
  const [mainSplineLoaded, setMainSplineLoaded] = useState(false);
  ...
  // Remplace cette URL par le lien d'intégration de ton animation Spline
    const SPLINE_URL = "https://my.spline.design/untitled-7UED11Rp7QpEDlkPuiz6jVVV/";
  // ==================== MOUNT ====================
  useEffect(() => {
    setIsMounted(true);
  }, []);
            ...
            <section className="hero">
              <div className="spline-container">
                <div className="spline-animation-area">
                  {/* Affiche le spinner tant que l'animation n'est pas chargée */}
                  {!mainSplineLoaded && (
                    <div className="spline-loader">
                      <div className="spinner"></div>
                    </div>
                  )}
                  {/* Error Boundary pour capturer les erreurs de Spline */}
                  <SplineErrorBoundary>
                    {isMounted && (
                      <Spline
                        scene={SPLINE_URL}
                        onLoad={() => setMainSplineLoaded(true)}
                      />
                    )}
                  </SplineErrorBoundary>
                </div>
